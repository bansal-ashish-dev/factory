...
export async function removeShippingMethods({
  cartId,
  shippingMethodIds,
}: {
  cartId: string
  ...
  return sdk.client
    .fetch(`/store/carts/${cartId}/shipping-methods`, {
      method: "DELETE",
      headers,
      body: { shipping_method_ids: shippingMethodIds },
    })
    .then(async () => {
      const cartCacheTag = await getCacheTag("carts")
      revalidateTag(cartCacheTag)
 ...
 * @param redirectOnSuccess - optional - Whether to redirect to the order confirmation page. Defaults to true.
...
export async function placeOrder(
  ...
  if (cartRes?.type === "order") {
    ...
    const [orderCacheTag, cartCacheTag, fulfillmentCacheTag] =
      await Promise.all([
        getCacheTag("orders"),
        getCacheTag("carts"),
    ...
    // Remove cart ID from cookies and clear cart data
    await removeCartId()
    ...
    return cartRes
  } else if (cartRes && typeof cartRes === "object" && "order_set" in cartRes) {
    // Handle orderset response for marketplace
    const orderSet = (cartRes as any).order_set
    if (orderSet && orderSet.orders && orderSet.orders.length > 0) {
      const firstOrder = orderSet.orders[0]
      const countryCode =
        firstOrder.shipping_address?.country_code?.toLowerCase() || "us"
      ...
      if (redirectOnSuccess) {
        redirect(
          `/${countryCode}/order/${firstOrder.id}/confirmed?orderset=${orderSet.id}`
        )
      }
...
export async function addToCartBulk({
  ...
  await fetch(
    ...
    .catch((error) => {
      console.error("Failed to add items to cart in bulk", error)
    })
}
