...
const ItemFull = ({
  ...
  const handleBlur = (value: number) => {
    ...
    // For bundle products, round to nearest bundle increment
    if (isBundleProduct) {
      const remainder = value % bundleSize
      if (remainder === 0) {
        value = value
      } else if (remainder >= bundleSize / 2) {
        value = value + (bundleSize - remainder)
      } else {
  ...
  // Get bundle size from variant metadata
  const bundleSize =
    parseInt(item.variant?.metadata?.bundle_quantity as string) || 1
  const isBundleProduct = bundleSize > 1

  // For bundle products, max quantity should be a multiple of bundle size
  const effectiveMaxQuantity = isBundleProduct
    ? Math.floor(maxQuantity / bundleSize) * bundleSize
    : maxQuantity
      ...
      <div className="flex gap-x-4 items-start">
        ...
        <div className="flex flex-col gap-y-2 justify-between min-h-full self-stretch">
          ...
          <div className="flex small:flex-row flex-col gap-2">
            ...
            <div className="flex gap-x-2">
              <div className="flex gap-x-3 shadow-[0_0_0_1px_rgba(0,0,0,0.1)] rounded-full w-fit p-px items-center">
                <button
                  ...
                  onClick={() => changeQuantity(item.quantity - bundleSize)}
                  disabled={item.quantity <= bundleSize || disabled}
                >
                  -
                </button>
                ...
                <button
                  ...
                  onClick={() => changeQuantity(item.quantity + bundleSize)}
                  disabled={item.quantity >= effectiveMaxQuantity || disabled}
                >
                  +
                </button>
              </div>
            ...
            />
            {isBundleProduct && (
              <div className="text-xs text-neutral-500 bg-neutral-100 px-2 py-1 rounded-md ml-2 flex flex-col items-center">
                <span>x {bundleSize} Pcs</span>
              </div>
            )}
          </div>
        </div>
